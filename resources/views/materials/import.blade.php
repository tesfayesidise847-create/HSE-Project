<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">{{ __('Import Materials') }}</h2>
    </x-slot>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6 overflow-hidden shadow-sm sm:rounded-lg bg-white dark:bg-gray-800">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                        {{ __('Download the Excel template (.xls), fill in material data using the exact column headers, then upload the file below.') }}
                    </p>
                    <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                        {{ __('Columns') }}: <strong>material_name</strong>, <strong>material_description</strong>, <strong>quantity</strong>, <strong>unit_of_measure</strong>
                    </p>
                    <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                        {{ __('Available values') }}: <strong>Pcs</strong>, <strong>Pair</strong>, <strong>Meter</strong>, <strong>Liter</strong>, <strong>Bag</strong>
                    </p>
                    <a href="{{ route('materials.import.template') }}" class="mt-4 inline-flex items-center rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-500">
            ...
            <div class="overflow-hidden shadow-sm sm:rounded-lg bg-white dark:bg-gray-800">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form method="POST" action="{{ route('materials.import.store') }}" enctype="multipart/form-data" class="mt-4">
                        @csrf
                        ...
                        <div class="mt-6 flex items-center gap-4">
                            <x-primary-button>{{ __('Import Materials') }}</x-primary-button>
